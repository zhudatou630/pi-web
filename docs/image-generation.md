...
工具提示词不写默认连接的 id，只让 Agent 省略 `connection`：Settings 里改默认连接后，执行层当场读到，不必等 `/reload`。
...
- 弹窗可粘贴、拖入或选一张原图，此时变为改图，只列出声明 editing 的连接。输入栏里恰好有一张已粘贴的图时，打开弹窗会自动带上；生成成功后从输入栏移除它。原图先存进 `.pi/generated-images/`，再按 `target` 改图，工具参数不变。只支持一张原图。
