...
「默认连接」可选已启用且有凭证的连接。Agent 未指定 `connection` 时，执行层优先使用默认连接；若其凭证不可用，按内置连接、再自定义连接的显示顺序选下一条可用且支持请求参数的连接。遇到上游凭证失效、配额不足或限流时继续尝试下一条；普通请求错误、超时和用户明确指定连接时不跨连接回退。输入栏生图弹窗会直接传入用户选定的连接。
