...
Every Chinese browser locale, including Traditional Chinese (`zh-TW`,
`zh-HK`, `zh-Hant`), resolves to `zh-CN`; there is no Traditional Chinese
package. A stored `pi-locale` of `zh-TW` from older versions also maps to
`zh-CN`.
...
The built-in `Locale` type currently lists `en` and `zh-CN` for
