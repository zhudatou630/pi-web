...
test("tracks the visual viewport while the software keyboard is open", () => {
  ...
  assert.match(cssSource, /\.workspace-header \{[^}]*height: calc\(var\(--workspace-header-height\) \+ env\(safe-area-inset-top\)\);[^}]*padding-top: env\(safe-area-inset-top\);/);
  assert.equal((appShellSource.match(/className="workspace-header"/g) ?? []).length, 2);
  assert.match(appShellSource, /\/\* Right panel tab bar \*\/[\s\S]*?className="workspace-header"/);
