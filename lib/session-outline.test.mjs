...
test("userMessagePreview collapses skill expansion tags into clean command previews", () => {
  const fullSkill = `<skill name="book-teacher" location="/home/user/SKILL.md">\nReferences are relative to /home/user.\n\nInstructions here\n</skill>\n\n继续精读第2章`;
  assert.equal(userMessagePreview(fullSkill), "/skill:book-teacher 继续精读第2章");

  const skillNoArgs = `<skill name="book-teacher" location="/home/user/SKILL.md">\nReferences are relative to /home/user.\n\nInstructions here\n</skill>`;
  assert.equal(userMessagePreview(skillNoArgs), "/skill:book-teacher");

  const unclosedSkill = `<skill name="book-teacher" location="/home/user/SKILL.md">\nSome user text`;
  assert.equal(userMessagePreview(unclosedSkill), "/skill:book-teacher Some user text");
});
...
test("buildSessionOutline keeps image-only user turns", () => {
  const entries = [
    { id: "u1", parentId: null, type: "message", timestamp: "t1", message: { role: "user", content: [{ type: "image" }] } },
  ];
  assert.deepEqual(buildSessionOutline(entries, "u1"), [{ entryId: "u1", preview: "…" }]);
});
