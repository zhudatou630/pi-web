...
test("reports an ambiguous bare exact model id instead of failing the whole list", async () => {
  // A bare id that matches two providers cannot be resolved, but it must not
  // take the rest of the scope down with it: the panel needs a list to show.
  const result = await resolveVisibleModels(runtime, ["claude-sonnet-5", "anthropic/claude-opus-5"]);
  assert.deepEqual(result.ambiguous, ["claude-sonnet-5"]);
...
test("reports an ambiguous bare exact model id with a thinking suffix", async () => {
  const result = await resolveVisibleModels(runtime, ["claude-sonnet-5:high"]);
  assert.deepEqual(result.ambiguous, ["claude-sonnet-5:high"]);
  // Nothing else was resolvable, so the list falls back to everything usable.
  assert.ok(refs(result).length > 0);
});
