...
test("resolves relative links outside the base directory", async () => {
  const { resolveLocalFileHref } = await loadSubject();
  ...
  assert.equal(
    resolveLocalFileHref("../file.js", "/home/me/project/docs/nested"),
...
test("a #page= selector survives the href-to-path resolution", async () => {
  const { resolveLocalFileHref, pdfPageFromHref } = await loadSubject();
  const href = "/repo/report.pdf#page=12";
  // The path resolution strips the fragment (it is not part of the path)...
  assert.equal(resolveLocalFileHref(href, "/repo"), "/repo/report.pdf");
  // ...and the page must be recoverable separately, or the viewer opens page 1.
  assert.equal(pdfPageFromHref(href), 12);
});

test("pdfPageFromHref accepts only a positive integer page selector", async () => {
  const { pdfPageFromHref } = await loadSubject();
  assert.equal(pdfPageFromHref("a.pdf#page=1"), 1);
  // Fragment keys are case-sensitive in the PDF spec; only lowercase `page` counts.
  assert.equal(pdfPageFromHref("a.pdf#PAGE=7"), null);
  assert.equal(pdfPageFromHref("a.pdf#page=0"), null);
  assert.equal(pdfPageFromHref("a.pdf#page=-3"), null);
  assert.equal(pdfPageFromHref("a.pdf#page=abc"), null);
  // A PDF fragment carries parameters, so read them as such.
  assert.equal(pdfPageFromHref("a.pdf#page=12&zoom=100"), 12);
  assert.equal(pdfPageFromHref("a.pdf#zoom=100&page=12"), 12);
  assert.equal(pdfPageFromHref("a.pdf#page=12&view=FitH"), 12);
  assert.equal(pdfPageFromHref("a.pdf#page=1.5"), null);
  assert.equal(pdfPageFromHref("a.pdf#page="), null);
  // Other fragments are not page selectors and must stay untouched.
  assert.equal(pdfPageFromHref("a.ts#L42"), null);
  assert.equal(pdfPageFromHref("a.md#section"), null);
  assert.equal(pdfPageFromHref("a.pdf"), null);
  assert.equal(pdfPageFromHref(undefined), null);
});
