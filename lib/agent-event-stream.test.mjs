...
function openStream(t, req, sessionId, sessionPromise) {
  const stream = createAgentEventStream(req, sessionId, sessionPromise);
  const reader = stream.getReader();
  t.after(() => { void reader.cancel(); });
  return reader;
}
...
test("opens the transport before a slow session is ready and snapshots after subscribing", async (t) => {
...
test("closes an established stream when its session is destroyed", async (t) => {
  let closeListener;
  let eventUnsubscribed = 0;
  let closeUnsubscribed = 0;
  const reader = openStream(
    t,
    ...
    Promise.resolve({
      ...
      onEvent() {
        return () => { eventUnsubscribed += 1; };
      },
      onClose(listener) {
        closeListener = listener;
        return () => { closeUnsubscribed += 1; };
      },
    }),
  );
  ...
  closeListener();
  assert.equal((await readWithin(reader)).done, true);
  assert.equal(eventUnsubscribed, 1);
  assert.equal(closeUnsubscribed, 1);
});
...
test("does not subscribe when the client cancels during startup", async (t) => {
  const startup = deferred();
  let subscribeCount = 0;
  const reader = openStream(
    t,
...
test("closes an already-aborted request and handles a later startup rejection", async (t) => {
  const abortController = new AbortController();
  abortController.abort();
  const reader = openStream(
    t,
...
test("SSE onEvent uses keepAlive true", async (t) => {
