...
const { createTerminal, shellEnvironment, getTerminalCwd, hasTerminal, killTerminal, subscribeTerminal, TERMINAL_RECONNECT_MS } = await jiti.import("./terminal-manager.ts");
const updateState = await jiti.import("./app-update-state.ts");
const { GET } = await jiti.import("../app/api/terminal/[id]/events/route.ts");

test("an accepted app update blocks terminal creation before loading the native module", () => {
  globalThis.__piWebUpdating = true;
  try {
    assert.throws(() => createTerminal(process.cwd(), 80, 24), /Pi Web is updating/);
  } finally {
...
test("native module load failures are deferred until creation and include repair instructions", async () => {
  ...
  runInNewContext(outputText, { exports, process, require(id) {
    if (id === "./app-update-state") return updateState;
...
test("shell environment defaults to UTF-8 locale when the host has none", async () => {
  ...
  const require = (id) => id === "./app-update-state" ? updateState : nodeRequire(id);
  const paths = await jiti.import("./paths.ts");
  const source = readFileSync(new URL("./terminal-manager.ts", import.meta.url), "utf8");
  const { outputText } = ts.transpileModule(source, { compilerOptions: { module: ts.ModuleKind.CommonJS, target: ts.ScriptTarget.ES2022 } });
  const spawnCalls = [];
...
test("terminal shells do not inherit this server's update rights or Next internals", () => {
  Object.assign(process.env, { PI_WEB_CAN_UPDATE: "1", PI_WEB_UPDATE_BLOCKED: "readonly", __NEXT_PRIVATE_ORIGIN: "http://x", PI_WEB_HOSTNAME: "h" });
  try {
    const env = shellEnvironment();
    assert.equal(env.PI_WEB_CAN_UPDATE, undefined);
    assert.equal(env.PI_WEB_UPDATE_BLOCKED, undefined);
    assert.equal(env.__NEXT_PRIVATE_ORIGIN, undefined);
    assert.equal(env.PI_WEB_HOSTNAME, "h");
  } finally {
    for (const key of ["PI_WEB_CAN_UPDATE", "PI_WEB_UPDATE_BLOCKED", "__NEXT_PRIVATE_ORIGIN", "PI_WEB_HOSTNAME"]) delete process.env[key];
  }
});
