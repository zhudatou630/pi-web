...
test("boolean skills/extensions aliases are seeded and whitelists are left alone", async () => {
  const cwd = await mkdtemp(join(tmpdir(), "pi-web-subagents-"));
  try {
    await mkdir(join(cwd, ".pi", "agents"), { recursive: true });
    await writeFile(
    ...
    assert.equal(loaded.loadSkills, true);
    assert.equal(loaded.loadExtensions, true);

    saveSubagentProfile(cwd, "project", { ...loaded, loadSkills: false, enabled: true });
    const source = await readFile(join(cwd, ".pi", "agents", "aliased.md"), "utf8");
    assert.match(source, /skills: false/);
    assert.match(source, /extensions: pi-advisor-flow/);
    assert.doesNotMatch(source, /extensions: false/);
  } finally {
    await rm(cwd, { recursive: true, force: true });
  }
});
