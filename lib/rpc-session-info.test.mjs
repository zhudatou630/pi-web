...
test("lists queued subagents from the live run registry", (t) => {
  const previousRegistry = globalThis.__piSessions;
  ...
  globalThis.__piSubagentRuns = new Map([[
    ...
    {
      run: {
        sessionId: "queued-subagent",
        sessionPath: "/tmp/queued-subagent.jsonl",
        parentSessionId: "parent-session",
        parentToolCallId: "tool-call",
        profile: "explore",
        description: "Inspect queued",
        task: "Inspect files",
        runInBackground: true,
        status: "queued",
        createdAt: "2026-08-12T01:02:04.000Z",
      },
    },
  ]]);
  t.after(() => {
  ...
  const [info] = getRpcSessionInfos();

  assert.equal(info.id, "queued-subagent");
  assert.equal(info.relation.status, "queued");
});

test("collapses skill expansions in transient session-list previews", (t) => {
  const previousRegistry = globalThis.__piSessions;
  const skillPrompt = `<skill name="review" location="/tmp/SKILL.md">
References are relative to /tmp.

${"large body\n".repeat(1000)}</skill>

src/main.ts`;
  const runtime = makeRuntimeSession({
    id: "skill-runtime",
    filePath: join(tmpdir(), "pi-web-missing-skill-session.jsonl"),
    running: true,
    entries: [{
      type: "message",
      id: "u1",
      ...
      message: { role: "user", content: skillPrompt },
    }],
  });
  globalThis.__piSessions = new Map([["skill-runtime", runtime]]);
  t.after(() => {
    globalThis.__piSessions = previousRegistry;
  });

  const [info] = getRpcSessionInfos();
  assert.equal(info.firstMessage, "/skill:review src/main.ts");
});
