...
test("seeds the first render from the last stored snapshot after a page load", async () => {
  const store = new Map();
  ...
  try {
    // A previous page load stored this snapshot; this module has never loaded the key.
    store.set("pi-web:models:/reloaded-project", JSON.stringify(modelsData("stored")));
    assert.deepEqual(peekModelsClientCache("/reloaded-project"), modelsData("stored"));

    // It only seeds the render: a load still fetches, and the fresh result is stored.
    let loads = 0;
    const fresh = await loadModelsWithClientCache("/reloaded-project", async () => {
      loads += 1;
      return modelsData("fresh");
    });
    assert.equal(loads, 1);
    assert.deepEqual(JSON.parse(store.get("pi-web:models:/reloaded-project")), fresh);
  } finally {
