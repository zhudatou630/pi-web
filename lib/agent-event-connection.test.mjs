...
test("forced reconnect replaces the source and isolates late events", async () => {
  const { connection, sources, events } = createHarness();
  const first = connection.ensureConnected("session-a");
  sources[0].open();
  sources[0].message({ type: "connected" });
  ...
  const replaced = connection.reconnect("session-a");
  assert.equal(sources.length, 2);
  assert.equal(sources[0].closeCount, 1);

  sources[0].message({ type: "notify", message: "old" });
  sources[1].open();
  sources[1].message({ type: "connected" });
  sources[1].message({ type: "notify", message: "new" });
  await replaced;

  assert.deepEqual(events.map((event) => event.type), ["connected", "connected", "notify"]);
  assert.equal(events.at(-1).message, "new");
});

test("manual close does not let a waiter follow a later connection", async () => {
  const { connection, sources } = createHarness();
  const abandoned = connection.ensureConnected("session-a");
  connection.close();
  const replacement = connection.ensureConnected("session-a");
...
test("same-session reconnect lets an in-flight waiter follow the new source", async () => {
  const { connection, sources } = createHarness();
  const first = connection.ensureConnected("session-a");
  sources[0].open();
...
test("same-session reconnect does not hide a startup error without a replacement", async () => {
  const { connection, sources } = createHarness();
  const first = connection.ensureConnected("session-a");
  sources[0].open();
  sources[0].message({ type: "startup_error", errorMessage: "broken config" });
