...
test("apply_patch contributes the files inside its patch document", () => {
  // The tool has no file_path argument, so without reading the patch its files
  // never reach the changed-file list.
  const patch = [
    "*** Begin Patch",
  ...
  const files = extractTurnWrittenFiles(
    [{ type: "toolCall", toolCallId: "c1", toolName: "apply_patch", input: { patch } }],
    new Map([["c1", { role: "toolResult", toolCallId: "c1", content: [], isError: false }]]),
    "/repo",
  );

  assert.deepEqual(files.map((f) => f.filePath), [
    "/repo/src/a.ts",
    "/repo/src/b.ts",
    // A move is reported at its destination: that is where the content now is.
    "/repo/src/moved.ts",
  ]);
});

test("a failed apply_patch writes nothing", () => {
  const patch = "*** Begin Patch\n*** Update File: src/a.ts\n@@\n-x\n+y\n*** End Patch";
  const files = extractTurnWrittenFiles(
    [{ type: "toolCall", toolCallId: "c1", toolName: "apply_patch", input: { patch } }],
    new Map([["c1", { role: "toolResult", toolCallId: "c1", content: [], isError: true }]]),
    "/repo",
  );
