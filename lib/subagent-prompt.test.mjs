...
test("replace prompt mode uses the profile as the exact system prompt and skips project instructions", () => {
  const plan = buildSubagentPromptPlan({
    profileSystemPrompt: "You are a reviewer.",
    projectInstructions: "Follow AGENTS.md.",
    tools: ["read"],
    promptMode: "replace",
  ...
  assert.equal(plan.chatOnly, false);
  assert.equal(plan.exactSystemPrompt, "You are a reviewer.\n\nParent context");
  assert.deepEqual(plan.appendSystemPrompt, ["You are a reviewer.", "Parent context"]);
  assert.equal(plan.delegatedTask, "Review the parser.");
});
