...
test("default connection must be enabled and follows changes to active connections", async (t) => {
  const root = await agentDir(t);
  writeImageGenerationSettings({ enabled: true }, root);
  writeImageGenerationSettings({ defaultConnection: "banana-2" }, root);
  assert.equal(resolveImageConfig(root).defaultConnection, "banana-2");
  assert.throws(() => writeImageGenerationSettings({ defaultConnection: "missing" }, root), /is not enabled/);
  writeImageGenerationSettings({ connections: { "banana-2": { enabled: false } } }, root);
  assert.equal(resolveImageConfig(root).defaultConnection, "chatgpt-flare");
});
