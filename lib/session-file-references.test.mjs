...
test("authorizes home-relative tilde paths for referenced external files", async () => {
  const { isFilePathReferencedByEntries } = await loadSubject();
  const entries = [
    {
      type: "message",
      ...
      message: {
        ...
        content: [
          {
            type: "text",
            text: "路径：~/.pi/handoff/2026-07-07-0139.md",
          },
        ],
      },
    },
  ...
  assert.equal(
    isFilePathReferencedByEntries("/home/me/.pi/handoff/2026-07-07-0139.md", entries, "/home/me"),
    true,
  );
});

test("does not authorize tilde sibling files by prefix match", async () => {
  const { isFilePathReferencedByEntries } = await loadSubject();
  const entries = [
    {
      type: "message",
      ...
      parentId: null,
      timestamp: "2026-01-01T00:00:00.000Z",
      message: { role: "assistant", content: "路径：~/.pi/handoff/2026-07-07-0139.md.bak" },
    },
  ];
