...
test("sums usage across ALL entries, including history compacted away", () => {
  ...
  assert.ok(Math.abs(stats.costs.input - 0.3) < 1e-9);
  assert.ok(Math.abs(stats.costs.cacheWrite - 0.02) < 1e-9);
...
test("includes cache-warming usage entries", () => {
  ...
  assert.equal(stats.tokens.total, 15);
