...
test("moves explorer across projects even when the current project has worktrees", () => {
  assert.equal(shouldAdoptSessionCwd({
    sessionCwd: "/another-repo",
    selectedCwd: main.path,
    sessionProjectKey: "/another-repo",
    selectedProjectKey: "/repo",
    worktrees,
  }), true);
});
