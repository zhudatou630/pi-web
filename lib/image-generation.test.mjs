...
const { extractMentionedImagePath, getImageGenerationResult, imageToolDisplayKind, splitImageMentions, imageRatioKind, xaiAspectRatio } = await jiti.import("./image-generation.ts");
...
test("the host extension stays absent without a runnable xAI connection", () => {
  const agentDir = mkdtempSync(path.join(tmpdir(), "pi-image-agent-"));
  try {
    writeFileSync(path.join(agentDir, "images.json"), JSON.stringify({
      connections: { proxy: { provider: "sub2api", model: "image" } },
    }));
    assert.equal(captureTool(agentDir), undefined);
  } finally {
    rmSync(agentDir, { recursive: true, force: true });
