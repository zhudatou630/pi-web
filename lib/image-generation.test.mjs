...
const { extractMentionedImagePath, getImageGenerationResult, IMAGE_CUSTOM_MODEL_PRESETS, imageConnectionTransport, imageDisplayRatio, imagePixelRatio, imageToolDisplayKind, splitImageMentions, imageRatioKind, xaiAspectRatio } = await jiti.import("./image-generation.ts");
...
const { buildOpenAIImagesBody, resolveOpenAIImagesUrl } = await jiti.import("./image-generation-openai-images.ts");
...
async function captureTool(agentDir) {
  let tool;
  await createImageGenerationExtension(agentDir, { hasAuth: () => true }).factory({ registerTool(value) { tool = value; } });
  return tool;
}

async function captureToolWithoutAuth(agentDir) {
  let tool;
  await createImageGenerationExtension(agentDir).factory({ registerTool(value) { tool = value; } });
  return tool;
}
...
test("the extension stays absent until image generation is enabled", async () => {
  const agentDir = mkdtempSync(path.join(tmpdir(), "pi-image-agent-"));
  try {
    assert.equal(await captureTool(agentDir), undefined);
  } finally {
    rmSync(agentDir, { recursive: true, force: true });
  }
});

test("the host extension stays absent without a runnable image connection", async () => {
  const agentDir = mkdtempSync(path.join(tmpdir(), "pi-image-agent-"));
  try {
    mkdirSync(path.join(agentDir, "images"));
    writeFileSync(path.join(agentDir, "images", "settings.json"), JSON.stringify({ enabled: true }));
    assert.equal(await captureTool(agentDir), undefined);
  } finally {
    rmSync(agentDir, { recursive: true, force: true });
  }
...
test("the extension stays absent when the settings switch is off", async () => {
  const agentDir = mkdtempSync(path.join(tmpdir(), "pi-image-agent-"));
  try {
    writeFileSync(path.join(agentDir, "images.json"), JSON.stringify(imageConfig()));
    mkdirSync(path.join(agentDir, "images"));
    writeFileSync(path.join(agentDir, "images", "settings.json"), JSON.stringify({ enabled: false }));
    assert.equal(await captureTool(agentDir), undefined);
  } finally {
    rmSync(agentDir, { recursive: true, force: true });
  }
...
test("the extension loads built-in connections from settings without images.json", async () => {
  const agentDir = mkdtempSync(path.join(tmpdir(), "pi-image-agent-"));
  try {
    mkdirSync(path.join(agentDir, "images"));
    writeFileSync(path.join(agentDir, "images", "settings.json"), JSON.stringify({
      enabled: true,
      connections: { "grok-imagine": { enabled: true } },
    }));
    assert.ok(await captureTool(agentDir));
  } finally {
    rmSync(agentDir, { recursive: true, force: true });
  }
});

test("the host extension stays absent when enabled connections have no credentials", async () => {
  const agentDir = mkdtempSync(path.join(tmpdir(), "pi-image-agent-"));
  try {
    writeFileSync(path.join(agentDir, "images.json"), JSON.stringify({
      connections: {
        studio: { provider: "unconfigured-image-provider", model: "gpt-image-2.5-flare" },
      },
    }));
    assert.equal(await captureToolWithoutAuth(agentDir), undefined);
  } finally {
    rmSync(agentDir, { recursive: true, force: true });
  }
});
...
test("OpenAI Images URLs stay on the relay /images path", () => {
  assert.equal(resolveOpenAIImagesUrl("https://relay.example/v1", "generations"), "https://relay.example/v1/images/generations");
  assert.equal(resolveOpenAIImagesUrl("https://relay.example/v1/", "edits"), "https://relay.example/v1/images/edits");
  assert.equal(resolveOpenAIImagesUrl("https://relay.example/v1/images", "edits"), "https://relay.example/v1/images/edits");
  const generated = buildOpenAIImagesBody("gpt-image-2.5-flare", "a square", undefined, "1:1", "medium");
  assert.deepEqual(generated, {
    prompt: "a square",
    model: "gpt-image-2.5-flare",
    size: "1024x1024",
  ...
  assert.equal("n" in generated, false);
  assert.equal("response_format" in generated, false);
  const edited = buildOpenAIImagesBody("gpt-image-2.5-sunburst", "add a border", { bytes: PNG, mimeType: "image/png" }, "auto", "low");
  assert.deepEqual(edited.images, [{ image_url: `data:image/png;base64,${PNG.toString("base64")}` }]);
});

test("the host extension loads an openai-codex connection without xAI", async () => {
  const agentDir = mkdtempSync(path.join(tmpdir(), "pi-image-agent-"));
  try {
    writeFileSync(path.join(agentDir, "images.json"), JSON.stringify({
      connections: {
...
test("the host extension loads a sub2api connection without xAI", async () => {
  const agentDir = mkdtempSync(path.join(tmpdir(), "pi-image-agent-"));
  try {
    writeFileSync(path.join(agentDir, "images.json"), JSON.stringify({
      connections: {
...
test("the host extension loads an antigravity connection without xAI", async () => {
  const agentDir = mkdtempSync(path.join(tmpdir(), "pi-image-agent-"));
  try {
    writeFileSync(path.join(agentDir, "images.json"), JSON.stringify({
      connections: {
...
test("image connection transport follows provider and model dialect", () => {
  assert.deepEqual(IMAGE_CUSTOM_MODEL_PRESETS.map((preset) => preset.model), [
    "gpt-image-2.5-flare",
    "gpt-image-2.5-sunburst",
    "grok-imagine-image-2.0",
  ]);
  assert.equal(imageConnectionTransport({ provider: "openai-codex", model: "gpt-image-2.5-flare" }), "codex");
  assert.equal(imageConnectionTransport({ provider: "antigravity", model: "gemini-3.1-flash-image" }), "antigravity");
  assert.equal(imageConnectionTransport({ provider: "xai", model: "grok-imagine-image-2.0" }), "xai");
  assert.equal(imageConnectionTransport({ provider: "my-relay", model: "grok-imagine-image-2.0" }), "xai");
  assert.equal(imageConnectionTransport({ provider: "my-relay", model: "gpt-image-2.5-flare" }), "openai-images");
});

test("the standard tool generates xAI images and rejects undeclared options", { timeout: 15_000 }, async () => {
  ...
  try {
    ...
    const tool = await captureTool(agentDir);
    assert.ok(tool);
    const context = {
      cwd,
    ...
    await assert.rejects(
      ...
      /No configured image connection supports size 2048x2048/,
    );
    await assert.rejects(
      tool.execute("resolution", { prompt: "too sharp", resolution: "4k" }, undefined, undefined, context),
      /No configured image connection supports resolution 4k/,
    );
    await assert.rejects(
...
test("automatic image routing skips unsigned connections and falls back only on quota errors", { timeout: 15_000 }, async () => {
  const root = mkdtempSync(path.join(tmpdir(), "pi-image-fallback-"));
  const agentDir = path.join(root, "agent");
  const cwd = path.join(root, "project");
  mkdirSync(agentDir);
  mkdirSync(cwd);
  ...
  const server = createServer(async (request, response) => {
    ...
    calls.push(provider);
    const failed = provider === "primary" && status !== 200;
    response.writeHead(failed ? status : 200, { "Content-Type": "application/json" });
    response.end(failed ? JSON.stringify({ error: { message: status === 400 ? "invalid request" : "quota exhausted" } }) : JSON.stringify({ data: [{ b64_json: PNG.toString("base64") }] }));
  });
  try {
    await new Promise((resolve) => server.listen(0, "127.0.0.1", resolve));
    const address = server.address();
    writeFileSync(path.join(agentDir, "images.json"), JSON.stringify({
      default: "unsigned",
      connections: Object.fromEntries(["unsigned", "primary", "backup"].map((id) => [id, {
        provider: id, model: "gpt-image-2.5-flare", capabilities: { editing: true, sizes: id === "backup" ? ["auto", "1:1"] : ["auto"] },
      }])),
    }));
    const tool = await captureTool(agentDir);
    const context = {
      ...
      sessionManager: { getBranch: () => [] },
      modelRegistry: {
        getProviderAuth: async (provider) => provider === "unsigned" ? undefined : { auth: { apiKey: "fixture-key" } },
        getProvider: (provider) => ({ baseUrl: `http://127.0.0.1:${address.port}/${provider}/v1` }),
      },
    };
    const result = await tool.execute("auto", { prompt: "a blue square", new_image: true }, undefined, undefined, context);
    assert.equal(result.details.connection, "backup");
    assert.deepEqual(calls, ["primary", "backup"]);

    calls.length = 0;
    await assert.rejects(tool.execute("explicit", { prompt: "a blue square", new_image: true, connection: "primary" }, undefined, undefined, context), /HTTP 429/);
    assert.deepEqual(calls, ["primary"]);

    calls.length = 0;
    status = 400;
    await assert.rejects(tool.execute("invalid", { prompt: "a blue square", new_image: true }, undefined, undefined, context), /HTTP 400/);
    assert.deepEqual(calls, ["primary"]);

    calls.length = 0;
    status = 401;
    const refreshed = await tool.execute("expired", { prompt: "a blue square", new_image: true }, undefined, undefined, context);
    assert.equal(refreshed.details.connection, "backup");
    assert.deepEqual(calls, ["primary", "backup"]);

    calls.length = 0;
    status = 200;
    const kept = await tool.execute("kept", { prompt: "a blue square", new_image: true, size: "1:1" }, undefined, undefined, context);
    assert.equal(kept.details.connection, "primary");
    assert.equal(kept.details.size, undefined);
    assert.deepEqual(calls, ["primary"]);

    calls.length = 0;
    await assert.rejects(
      tool.execute("explicit-size", { prompt: "a blue square", new_image: true, size: "1:1", connection: "primary" }, undefined, undefined, context),
      /does not support size 1:1/,
    );
...
test("the standard tool generates and edits Antigravity Banana 2 images", { timeout: 15_000 }, async () => {
  ...
  try {
    ...
    const context = {
      ...
      sessionManager: { getBranch: () => [] },
      modelRegistry: {
        getProviderAuth: async () => ({ auth: { apiKey: JSON.stringify({ token: "ya29.fixture-token", projectId: "proj-1" }) } }),
        getProvider: () => ({ baseUrl: `http://127.0.0.1:${address.port}` }),
      },
    };
...
test("Antigravity moves past a silent endpoint and names the endpoint on stream timeout", { timeout: 10_000 }, async () => {
  const { requestImageFromEndpoints } = await jiti.import("./image-generation-antigravity.ts");
  ...
  const working = await listen((_request, response) => {
    response.writeHead(200, { "Content-Type": "text/event-stream" });
    response.end(`data: ${JSON.stringify({ response: { candidates: [{ content: { parts: [{ inlineData: { data: PNG.toString("base64") } }] } }] } })}\n\n`);
  });
  try {
    const image = await requestImageFromEndpoints([silent.url, working.url], {}, "{}", undefined, { connectMs: 200, totalMs: 5_000 });
    assert.deepEqual(image, PNG);
    await assert.rejects(
      requestImageFromEndpoints([silent.url], {}, "{}", undefined, { connectMs: 200, totalMs: 5_000 }),
      new RegExp(`${silent.url} sent no response within 0.2s`),
    );
    await assert.rejects(
      requestImageFromEndpoints([stalled.url, working.url], {}, "{}", undefined, { connectMs: 200, totalMs: 500 }),
      new RegExp(`${stalled.url} timed out after 0.5s while streaming the image`),
    );
  } finally {
    for (const { server } of [silent, stalled, working]) {
      server.closeAllConnections();
      await new Promise((resolve) => server.close(resolve));
    }
  }
...
test("dialog source images are stored under the cwd as edit targets", async () => {
  const { saveSourceImage } = await jiti.import("./image-generation-runtime.ts");
  const cwd = mkdtempSync(path.join(tmpdir(), "pi-image-source-"));
  try {
    const target = await saveSourceImage(cwd, { data: PNG.toString("base64"), mimeType: "image/png" });
    assert.match(target, /^\.pi\/generated-images\/image-.+\.png$/);
    assert.deepEqual(readFileSync(path.join(cwd, target)), PNG);
    await assert.rejects(saveSourceImage(cwd, { data: Buffer.from("not an image").toString("base64"), mimeType: "image/png" }), /Unsupported image/);
  } finally {
