...
test("automatic image routing skips unsigned connections and falls back only on quota errors", { timeout: 15_000 }, async () => {
  ...
  const server = createServer(async (request, response) => {
    ...
    calls.push(provider);
    response.writeHead(provider === "primary" ? status : 200, { "Content-Type": "application/json" });
    response.end(provider === "primary" ? JSON.stringify({ error: { message: status === 400 ? "invalid request" : "quota exhausted" } }) : JSON.stringify({ data: [{ b64_json: PNG.toString("base64") }] }));
  });
  try {
    await new Promise((resolve) => server.listen(0, "127.0.0.1", resolve));
    const address = server.address();
    ...
    calls.length = 0;
    status = 429;
    const compatible = await tool.execute("compatible", { prompt: "a blue square", new_image: true, size: "1:1" }, undefined, undefined, context);
    assert.equal(compatible.details.connection, "backup");
