...
test("36px directory rows use a bounded window, including fractional/bottom scroll", () => {
  assert.deepEqual(outlineWindow(0, 0, 360), { start: 0, end: 0 });
  assert.deepEqual(outlineWindow(3, 0, 360), { start: 0, end: 3 });
  assert.deepEqual(outlineWindow(10000, 0, 360), { start: 0, end: 11 });
  assert.deepEqual(outlineWindow(10000, 180005, 360), { start: 4999, end: 5012 });
  assert.deepEqual(outlineWindow(10000, 999999, 360), { start: 9989, end: 10000 });
  assert.deepEqual(outlineWindow(10000, -100, 360), { start: 0, end: 11 });
});

test("keyboard navigation computes only the directory scroll offset", () => {
  assert.equal(revealOutlineEntry(5, 0, 360), 0);
  assert.equal(revealOutlineEntry(10, 0, 360), 36);
  assert.equal(revealOutlineEntry(9999, 0, 360), 359640);
  assert.equal(revealOutlineEntry(4, 200, 360), 144);
});
