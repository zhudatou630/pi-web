...
import { findActiveUser, mapEntriesToUsers, markerWindow, minimapReadingLine, revealOutlineEntry, outlineWindow } from "./chat-minimap.ts";
...
test("short last turn at the transcript tail activates the last user", () => {
  ...
  assert.equal(minimapReadingLine(191, 500, 700), 239);
  assert.equal(minimapReadingLine(192, 500, 700), Number.POSITIVE_INFINITY);
});
