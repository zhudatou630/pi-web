...
test("detects markdown and html preview paths", async () => {
  const { isFilePreviewPath } = await loadSubject();

  assert.equal(isFilePreviewPath("/tmp/README.MD"), true);
  assert.equal(isFilePreviewPath("C:\\Users\\me\\page.HTML"), true);
  assert.equal(isFilePreviewPath("/tmp/notes.mdx"), true);
  assert.equal(isFilePreviewPath("/tmp/main.ts"), false);
});
