...
test("an accepted app update blocks new sessions and commands on existing sessions", async () => {
  const wrapper = new AgentSessionWrapper(makePromptInner(() => {
    assert.fail("must not dispatch a prompt during update");
  }));
  globalThis.__piWebUpdating = true;
  try {
    await assert.rejects(wrapper.send({ type: "prompt", message: "hello" }), /Pi Web is updating/);
    await assert.rejects(startRpcSession("new", "", "/tmp"), /Pi Web is updating/);
  } finally {
    delete globalThis.__piWebUpdating;
    wrapper.destroy();
  }
});
...
test("a pre-completion gate can synchronously start follow-up work before completion is reported", async (t) => {
  let finishPrompt;
  let sdkListener;
  ...
  const wrapper = new AgentSessionWrapper(inner, {
    beforeAgentRunComplete: () => deferCompletion,
    onAgentRunComplete: (sessionId) => completed.push(sessionId),
  });
  t.after(() => wrapper.destroy());
  wrapper.start();
...
test("a stopped prompt completion does not reopen automatic follow-ups", async (t) => {
  let finishPrompt;
  const inner = makePromptInner((_message, options) => new Promise((resolve) => {
    finishPrompt = resolve;
    options.preflightResult(true);
  }));
  inner.sessionId = "session-stop-completion";
  inner.abort = () => Promise.resolve();
  const wrapper = new AgentSessionWrapper(inner);
  t.after(() => wrapper.destroy());
  ...
  finishPrompt();
  await nextTurn();
  assert.equal(wrapper.canStartBackgroundFollowUp(), false);
});

test("Stop pauses automatic follow-ups until the next accepted user prompt", async (t) => {
  ...
  assert.equal(wrapper.canStartBackgroundFollowUp(), true);
  await wrapper.send({ type: "abort" });
  assert.equal(wrapper.canStartBackgroundFollowUp(), false);

  wrapper.destroy();
  const replacement = new AgentSessionWrapper(inner);
  t.after(() => replacement.destroy());
  assert.equal(replacement.canStartBackgroundFollowUp(), false);
  await replacement.send({ type: "prompt", message: "continue" });
  assert.equal(replacement.canStartBackgroundFollowUp(), true);
  await replacement.send({ type: "abort_bash" });
  assert.equal(replacement.canStartBackgroundFollowUp(), false);
});
...
test("shutdown closes command admission before awaiting extension cleanup", async () => {
  let finishShutdown;
  const inner = makePromptInner(() => Promise.resolve());
  inner.extensionRunner.emit = () => new Promise((resolve) => { finishShutdown = resolve; });
  const wrapper = new AgentSessionWrapper(inner);

  const shutdown = wrapper.shutdown();
  assert.equal(wrapper.isClosing(), true);
  await assert.rejects(wrapper.send({ type: "get_state" }), /Session is closing/);
