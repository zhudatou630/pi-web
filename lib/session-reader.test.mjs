...
test("hides transcript system and usage entries from chat history", () => {
  const entries = [
    userEntry("u1", null, "hello"),
    {
      ...
      message: { role: "system", content: "internal prompt", timestamp: 1 },
    },
    {
      type: "usage",
      id: "usage1",
      ...
      provider: "test",
      model: "test-model",
      usage: { input: 1, output: 1, cacheRead: 1, cacheWrite: 0, cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, total: 0 } },
    },
    assistantEntry("a1", "usage1", "done"),
  ];

  const context = buildSessionContext(entries, "a1");
...
test("attaches assistant completedAt from the jsonl entry without rewriting start time", () => {
  ...
  assert.equal(context.messages[1].timestamp, startedAt);
  assert.equal(context.messages[1].completedAt, Date.parse("2026-01-01T00:00:08.000Z"));
});
...
test("live subagent status overrides an interrupted disk marker", () => {
  const base = {
    path: "/tmp/subagent.jsonl",
    id: "subagent",
    cwd: "/tmp",
    created: "2026-01-01T00:00:00.000Z",
    modified: "2026-01-01T00:00:01.000Z",
  ...
  const live = {
    ...base,
    relation: { ...base.relation, status: "queued" },
  };

  const [merged] = mergeSessionLists([base], [live]);

  assert.equal(merged.relation.status, "queued");
  assert.equal(merged.path, base.path);
});
