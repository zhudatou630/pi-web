...
function launcher(canUpdate = true) {
  ...
  const customRequire = Object.assign((name) => {
    ...
    if (name === "./app-update") return { getGlobalNpmCli: () => canUpdate ? { npmCli: "/npm-cli.js" } : undefined };
...
test("failed installation keeps serving the previous version", () => {
  const { children, logs, exits } = launcher();
  children[0].emit("message", { type: "pi-web:update" });
  children[0].emit("exit", 143, null);
  children[1].emit("exit", 1, null);
  assert.equal(children.length, 3, "restarts the old version instead of dying");
  assert.deepEqual(children[2].args, children[0].args);
  assert.deepEqual(exits, [], "stays up while the recovered server runs");
...
test("only the npm global install is eligible, excluding Git and pm2", () => {
  ...
  try {
    ...
    assert.deepEqual(getGlobalNpmCli(pkg, env), { npmCli });
    assert.equal(getGlobalNpmCli(temp, env), undefined);
    // systemd is allowed: the launcher respawns the child in place, so the
    // supervised main process never exits during an update.
    assert.deepEqual(getGlobalNpmCli(pkg, { ...env, INVOCATION_ID: "service" }), { npmCli });
...
test("read-only global installs surface a sudo hint", () => {
  // root and Windows ignore file modes for access(), so the EACCES path
  // cannot be simulated there.
  if (process.platform === "win32") return;
  if (typeof process.getuid === "function" && process.getuid() === 0) return;
  const temp = mkdtempSync(path.join(tmpdir(), "pi-web-update-ro-"));
  try {
    const root = path.join(temp, "global");
    const pkg = path.join(root, "@calmabacus", "pi-web");
    const npmCli = path.join(temp, "npm");
    mkdirSync(pkg, { recursive: true });
    mkdirSync(path.dirname(npmCli), { recursive: true });
    writeFileSync(npmCli, `console.log(${JSON.stringify(root)});`);
    const env = { ...process.env, PATH: temp, pm_id: "", PM2_HOME: "" };
    chmodSync(root, 0o555);
    assert.deepEqual(getGlobalNpmCli(pkg, env), { reason: "readonly" });
  } finally {
    chmodSync(path.join(temp, "global"), 0o755);
    rmSync(temp, { recursive: true, force: true });
  }
});
