...
test("status and array widgets follow the RPC fire-and-forget protocol", (t) => {
  ...
  ui.setWidget("factory", () => { factoryCalled = true; return {}; });
  ...
  assert.equal(events.some((event) => event.type === "extension_widgets"), false);
});

test("keepAlive subscribers keep idle runtime from disposal", async (t) => {
  ...
  t.after(() => wrapper.destroy());
  wrapper.start();
  const unsub = wrapper.onEvent(() => {}, true);
