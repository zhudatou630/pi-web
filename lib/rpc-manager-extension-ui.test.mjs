...
test("status and array widgets follow the RPC fire-and-forget protocol", (t) => {
  const { ui, events } = setup(t);

  ui.setStatus("sync", "Ready");
  ui.setStatus("sync", undefined);
  ui.setWidget("summary", ["one", "two"], { placement: "belowEditor" });
  ui.setWidget("summary", undefined);

  const requests = events
    .filter((event) => event.method === "setStatus" || event.method === "setWidget")
    .map(({ id, ...event }) => {
      assert.equal(typeof id, "string");
      return event;
    });
  assert.deepEqual(requests, [
    { type: "extension_ui_request", method: "setStatus", statusKey: "sync", statusText: "Ready" },
    { type: "extension_ui_request", method: "setStatus", statusKey: "sync", statusText: undefined },
    { type: "extension_ui_request", method: "setWidget", widgetKey: "summary", widgetLines: ["one", "two"], widgetPlacement: "belowEditor" },
    { type: "extension_ui_request", method: "setWidget", widgetKey: "summary", widgetLines: undefined, widgetPlacement: undefined },
  ]);
});

test("factory widgets render through the headless tui and dispose on clear", (t) => {
  const { ui, events } = setup(t);
  let disposed = 0;
  let tui;
  let value = "one";
  ui.setWidget("todos", (received) => {
    tui = received;
    return {
      render: () => [value],
      dispose: () => { disposed += 1; },
  ...
  const widgetEvents = () => events.filter((event) => event.method === "setWidget");
  assert.deepEqual(widgetEvents().at(-1).widgetLines, ["one"]);
  value = "two";
  tui.requestRender();
  assert.deepEqual(widgetEvents().at(-1).widgetLines, ["two"]);
  ui.setWidget("todos", undefined);
  assert.equal(disposed, 1);
  const afterClear = widgetEvents().length;
  tui.requestRender();
  assert.equal(widgetEvents().length, afterClear);
});

test("stale factory requestRender does not update after replace", (t) => {
  const { ui, events } = setup(t);
  let oldTui;
  ui.setWidget("todos", (tui) => {
    oldTui = tui;
    return { render: () => ["old"] };
  });
  ui.setWidget("todos", () => ({ render: () => ["new"] }));
  const count = events.filter((event) => event.method === "setWidget").length;
  oldTui.requestRender();
  assert.equal(events.filter((event) => event.method === "setWidget").length, count);
  assert.deepEqual(events.filter((event) => event.method === "setWidget").at(-1).widgetLines, ["new"]);
});

test("late event subscribers receive the current widget snapshot", (t) => {
  const { ui, wrapper } = setup(t);
  ui.setWidget("todos", ["a"]);
  const late = [];
  wrapper.onEvent((event) => late.push(event));
  assert.deepEqual(
    late.filter((event) => event.method === "setWidget").at(-1).widgetLines,
    ["a"],
  );
});

test("empty factory render keeps the last paper and later requestRender can update", (t) => {
  const { ui, events } = setup(t);
  let tui;
  let lines = ["keep"];
  ui.setWidget("todos", (received) => {
  ...
  lines = [];
  tui.requestRender();
  assert.deepEqual(events.filter((event) => event.method === "setWidget").at(-1).widgetLines, ["keep"]);
  lines = ["again"];
  tui.requestRender();
  assert.deepEqual(events.filter((event) => event.method === "setWidget").at(-1).widgetLines, ["again"]);
});
