...
test("a background report announces that it is not a user message (#875)", (t) => {
  // Pi's `convertToLlm` replays a `custom` message as a plain user turn, so a
  // background report without this marker is read as user intent — and a
  // compaction pass then files it under Goal / Constraints.
  const header = getSubagentNotificationHeader();
  assert.ok(header.includes("not a message from the user"));

  const run = completedRun({ result: "Parser is in lib/parser.ts" });
  installPendingWave(t, run);
  const sent = [];
  const controller = createSubagentController({
    getSession: () => ({
      ...
      isClosing: () => false,
      isRunning: () => false,
      canStartBackgroundFollowUp: () => true,
      inner: { sendCustomMessage: (...args) => { sent.push(args); return Promise.resolve(); } },
    }),
    registerSession: () => {},
    resolveSessionPath: async () => null,
  ...
  const delivered = sent.find(([message]) => message.customType === "pi-web:subagent-notification");
  assert.ok(delivered, "the report must be delivered");
  const text = delivered[0].content;
  assert.ok(text.startsWith(header), "the marker must lead the message");
  assert.ok(text.includes("Parser is in lib/parser.ts"), "the report body must survive");
});
