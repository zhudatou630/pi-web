...
const {
  ...
  modelPickerRef,
  samePickerPatterns,
  removeExactProviderPatterns,
  countExactProviderPatterns,
} = await loadSubject();
...
test("disconnect strips exact provider refs and thinking pins, keeps globs", () => {
  const patterns = [
    "openai-codex/gpt-6-astra",
  ...
  assert.deepEqual(removeExactProviderPatterns(patterns, "openai-codex"), [
    "openai-codex/*",
    "sub2api/gpt-6-astra",
    "xai/grok-4.6",
  ]);
  assert.equal(countExactProviderPatterns(patterns, "openai-codex"), 2);
  assert.deepEqual(removeExactProviderPatterns(undefined, "openai-codex"), []);
  assert.equal(samePickerPatterns(
    undefined,
    removeExactProviderPatterns(undefined, "openai-codex"),
  ), true);
});
