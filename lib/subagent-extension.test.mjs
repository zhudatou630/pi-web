...
test("get_subagent_result covers missing, running, completed, failed, and aborted runs", async () => {
  ...
  await assert.rejects(tool.execute("call", { agent_id: "missing" }), /not found/);

  current = run();
  let result = await tool.execute("call", { agent_id: "child-session" });
  ...
  await assert.rejects(tool.execute("call", { agent_id: "child-session" }), /boom/);

  current = run({ status: "failed", error: "boom", result: "partial" });
  await assert.rejects(tool.execute("call", { agent_id: "child-session" }), /partial[\s\S]*boom/);
...
test("steer_subagent returns success and runtime failures", async () => {
  ...
  await assert.rejects(
    tool.execute("call", { agent_id: "child-session", message: "fail" }),
    /not running/,
  );
});
