...
test("built-in subagents persist their selected resource policy", async () => {
  ...
  assert.match(subagentSource, /SessionManager\.create\(/);
...
test("normal session teardown paths use graceful extension shutdown", async () => {
  ...
  assert.match(forkSource, /if \(!existsSync\(newSessionFile\)\)/);
  ...
  assert.match(deleteRouteSource, /await reserveRpcSessionFileMutation\(affected\.map/);
...
test("prompt preflight relies on Pi's transcript prompt state", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
...
test("normal sessions restore persisted tool selections before loading resources", async () => {
  ...
  // The chat-only prompt is resolved per run so a reload re-reads the context files.
  assert.match(startupSource, /createExactSystemPromptExtension\(readChatOnlySystemPrompt\)/);
  assert.match(startupSource, /loadProjectContextFiles\(\{ cwd: sessionCwd, agentDir \}\)/);
