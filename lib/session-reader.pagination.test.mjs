...
test("sliceActiveBranch walks from leaf back toward root, not forward", () => {
  ...
  assert.deepEqual(ids.slice(-3), ["e3", "e4", "e5"]);
  assert.equal(ids[0], "e2");
  assert.ok(!ids.includes("e6"));
});
...
test("tail window extends back to the user that starts the cut turn", () => {
  const entries = [
    { id: "u0", parentId: null, type: "message", timestamp: "t0", message: { role: "user", content: "older" } },
    { id: "a0", parentId: "u0", type: "message", timestamp: "t1", message: { role: "assistant", content: "older answer" } },
    { id: "u1", parentId: "a0", type: "message", timestamp: "t2", message: { role: "user", content: "now" } },
  ];
  for (let i = 0; i < 80; i++) {
    entries.push({
      id: `t${i}`,
      ...
      type: "message",
      timestamp: `t${3 + i}`,
      message: { role: "assistant", content: [{ type: "toolCall", toolCallId: `c${i}`, toolName: "bash", input: {} }] },
    });
  }
  const leaf = "t79";
  const sliced = sliceActiveBranch(entries, leaf, 50);
  assert.equal(sliced[0].id, "u1");
  assert.equal(sliced[sliced.length - 1].id, leaf);
  assert.equal(sliced.length, 81);
  const ctx = buildSessionContext(entries, leaf, { tail: 50 });
  assert.equal(ctx.entryIds[0], "u1");
  assert.equal(ctx.messages[0].role, "user");
  assert.equal(ctx.hasMore, true);
});

test("tail window extends back to a compaction anchor", () => {
  const entries = [
    { id: "u0", parentId: null, type: "message", timestamp: "t0", message: { role: "user", content: "old" } },
    { id: "compact", parentId: "u0", type: "compaction", timestamp: "t1", summary: "summary", firstKeptEntryId: "a0", tokensBefore: 10 },
  ];
  for (let i = 0; i < 60; i++) {
    entries.push({
      id: `a${i}`,
      parentId: i === 0 ? "compact" : `a${i - 1}`,
      type: "message",
      timestamp: `t${2 + i}`,
      message: { role: "assistant", content: "step" },
    });
  }
  const sliced = sliceActiveBranch(entries, "a59", 50);
  assert.equal(sliced[0].id, "compact");
  assert.equal(sliced.length, 61);
});

test("tail window stops extending at the 1000-entry cap", () => {
  const entries = [];
  ...
  const sliced = sliceActiveBranch(entries, "a1199", 50);
  assert.equal(sliced.length, 1000);
  assert.equal(sliced[0].id, "a200");
  assert.equal(sliced[sliced.length - 1].id, "a1199");
});
...
test("pagination loses no history when compaction kept entries fall outside the page", () => {
  ...
  for (const tail of [1, 3, 5, 50]) {
    ...
    while (page.hasMore) {
      ...
      assert.ok(page.entryIds.length > 0);
      assert.ok(page.entryIds.length <= Math.max(tail, 1000));
...
test("session stats cover the full file independently of the displayed tail", () => {
  ...
  assert.deepEqual(computeSessionStats(entries), {
    ...
    cost: 2,
    // The fixture only records cost.total; per-class splits are covered in session-stats.test.mjs.
