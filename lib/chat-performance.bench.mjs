...
function reduceStream(tabCount = 1, batchSize = 1) {
  for (let tab = 0; tab < tabCount; tab++) {
    ...
    for (let offset = 0; offset < markdown.length; offset += STREAM_CHUNK_CHARS) {
      ...
      if (events.length === batchSize || offset + STREAM_CHUNK_CHARS >= markdown.length) {
        state = streamReducer(state, batchSize === 1
          ? { type: "delta", event: events[0] }
          : { type: "deltas", events });
        events = [];
      }
    }
...
const rows = [
  ...
  measure("stream reducer, 16-delta batches", () => reduceStream(1, 16)),
