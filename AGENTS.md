...
- Three font roles, chosen by what the text *is*, not by which characters it contains:
  ...
  - **Code** `--font-mono` (Sarasa Term SC): only (a) verbatim content — code blocks, inline code, terminal, file viewer, diffs, tool args/output in chat, ASCII diagrams, system prompt, boxed verbatim values (device code, trust-dialog cwd, slash-command usage signature) — and (b) inputs where an identifier is typed (paths, URLs, model ids, headers, branch names, bash-mode composer). Identifiers shown as labels, list rows, descriptions, headers, or property values (package names, sources, paths, versions, branches, IDs, command names, file names) are UI: mixing mono into a UI line made rows look disordered. Any code face must keep CJK = exactly 2 Latin columns (Sarasa Term, LXGW WenKai Mono), so a Latin-only mono is not acceptable.
- Font schemes (`hooks/useFontScheme.ts`, localStorage `pi-font-scheme`, applied pre-paint as `html[data-font]` by `app/layout.tsx`) only redefine `--font-ui` / `--font-chat` / `--font-mono` in `app/globals.css`; components never branch on the scheme. `sarasa` (default) and `claude` exist today. `claude` only swaps prose to serif faces: Source Serif 4 (Latin-only variable subset, 400–600, no italic) in front of Noto Serif SC (variable 400–600, ~10 MB), mirroring claude.ai's serif Latin + Song CJK; CJK-shared punctuation goes to the Song face via `unicode-range`. UI and code stay Sarasa. `@font-face` downloads lazily, so only the selected scheme's faces are fetched.
