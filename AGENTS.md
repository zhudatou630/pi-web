...
- Three font roles, chosen by what the text *is*, not by which characters it contains:
  ...
  - **Prose** `--font-chat` (Sarasa UI SC today; future schemes may swap it for a serif/WenKai): user/assistant messages, thinking text, compaction/custom/subagent markdown, the composer, read documents (markdown file preview). Never hardcode prose to `--font-ui`.
  - **Code** `--font-mono` (Sarasa Term SC): anything whose meaning or layout depends on fixed width or that is copied/typed verbatim — code blocks, inline code, terminal, file viewer, diffs, bash-mode input, tool args/output, ASCII diagrams, frontmatter keys, and literal identifiers (paths, IDs, branch names, commands, model ids, tool/parameter names, types). Exception: the tool name in a chat process step is a row label (UI), matching the Thinking step; its argument preview stays code. Any code face must keep CJK = exactly 2 Latin columns (Sarasa Term, LXGW WenKai Mono), so a Latin-only mono is not acceptable.
